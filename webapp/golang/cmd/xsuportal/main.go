...
func main() {
	srv := echo.New()
	srv.Logger.SetLevel(gommonlog.OFF)
...
func (*AdminService) ListClarifications(e echo.Context) error {
	...
	if len(teamIds) > 0 {
		sql, params, err := sqlx.In("SELECT * FROM `teams` WHERE `id` IN (?) ", teamIds)
		if err != nil {
			return fmt.Errorf("get team(id=%v): %w", teamIds, err)
		}
		if err := db.Select(&teams, sql, params...); err != nil {
			return fmt.Errorf("get team(id=%v): %w", teamIds, err)
		}
	}
...
func (*AdminService) RespondClarification(e echo.Context) error {
	...
	if err := notifier.NotifyClarificationAnswered(db, db, &clarification, updated); err != nil {
...
func (*ContestantService) ListNotifications(e echo.Context) error {
	...
	/*
		...
		err = sqlx.Get(
			db,
			&found,
			"SELECT 1 FROM `push_subscriptions` WHERE `contestant_id` = ? LIMIT 1",
			contestant.ID,
		)
...
func (*AudienceService) Dashboard(e echo.Context) error {
	leaderboard, err := makeAudienceLeaderboardPB(e)
	if err != nil {
		return fmt.Errorf("make leaderboard: %w", err)
	}
...
func getCurrentContestant(e echo.Context, db sqlx.Queryer, lock bool) (*xsuportal.Contestant, error) {
	...
	// contestant, err := xsuportal.GetContestantByID(contestantID.(string), db, lock)
	if err == sql.ErrNoRows {
		return nil, nil
	}
	if err != nil {
...
// makeLeaderboardPB のAudience専用版
func makeAudienceLeaderboardPB(e echo.Context) (*resourcespb.Leaderboard, error) {
	contestStatus, err := getCurrentContestStatus(e, db)
	if err != nil {
		return nil, fmt.Errorf("get current contest status: %w", err)
	}
	...
	err = tx.Select(&leaderboard, query, contestFinished, contestFreezesAt, contestFinished, contestFreezesAt)
	if err != sql.ErrNoRows && err != nil {
	...
	err = tx.Select(&jobResults, jobResultsQuery, contestFinished, contestFreezesAt)
	if err != sql.ErrNoRows && err != nil {

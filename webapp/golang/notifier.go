...
func (n *Notifier) NotifyClarificationAnswered(db sqlx.Ext, db2 *sqlx.DB, c *Clarification, updated bool) error {
	...
	var contestantIds []interface{}
	for _, v := range contestants {
		contestantIds = append(contestantIds, v.ID)
	}
	var pushs []PushSubscription
	if len(contestantIds) > 0 {
		sql, params, err := sqlx.In("SELECT * FROM `push_subscriptions` WHERE `contestant_id` IN (?) ", contestantIds)
		if err != nil {
			return nil // TODO ほんとに大丈夫?
		}
		if err := db2.Select(&pushs, sql, params...); err != nil {
			return nil // TODO ほんとに大丈夫?
		}
	}
	...
	for _, contestant := range contestants {
		...
		if n.VAPIDKey() != nil {
			...
			for _, push := range pushs {
				if push.ContestantID == contestant.ID {
			...
			// Send Notification
			resp, err := webpush.SendNotification(message, s, n.options)
			if err != nil {
				return fmt.Errorf("push notif err")
			}
